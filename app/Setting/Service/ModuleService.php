...
class ModuleService extends AbstractService implements ModuleServiceInterface
{
    ...
    public function __construct(MsPro $mspro)
    {
        $this->mspro = $mspro;
    }
    ...
    {
        try {
            $migrateCommand = [ 'command' => 'mspro:migrate-run', 'name' => $name ];
            $seedCommand = [ 'command' => 'mspro:seeder-run', 'name' => $name ];
    ...
    public function getModuleCache(?string $moduleName = null): array
    {
        ...
        if ($data = $redis->get($key)) {
            ...
            $this->setModuleCache();
            $this->mspro->scanModule();
            return $this->mspro->getModuleInfo();
        }
    }
