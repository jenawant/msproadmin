...
use MsPro\Generator\ApiGenerator;
use MsPro\Generator\ControllerGenerator;
use MsPro\Generator\DtoGenerator;
use MsPro\Generator\MapperGenerator;
use MsPro\Generator\ModelGenerator;
...
class SettingGenerateTablesService extends AbstractService
{
    ...
    {
        // 非系统数据源，同步远程库的表结构到本地
        if ($params['source'] !== \MsPro\MsPro::getMsProName()) {
    ...
    protected function generateCodeFile(int $id, int $adminId): SettingGenerateTables
    {
        ...
        foreach ($classList as $cls) {
            $class = make($cls);
            if (get_class($class) == 'MsPro\Generator\SqlGenerator'){
    ...
    protected function packageCodeFile(): string
    {
        ...
        $zipFileName = BASE_PATH. '/runtime/msproadmin.zip';
