...
class SystemAppService extends AbstractService
{
    ...
    public function getAccessToken(array $params): array
    {
        if (empty($params['app_id'])) {
            throw new NormalStatusException(t('msproadmin.api_auth_fail'), MsProCode::API_APP_ID_MISSING);
        }

        if (empty($params['signature'])) {
            throw new NormalStatusException(t('msproadmin.api_auth_fail'), MsProCode::API_SIGN_MISSING);
        }
        ...
        if (! $model) {
            throw new NormalStatusException(t('msproadmin.access_denied'), MsProCode::API_AUTH_EXCEPTION);
        }

        if ($params['signature'] !== $this->getSignature($model['app_secret'], $params)) {
            throw new NormalStatusException(t('msproadmin.api_auth_fail'), MsProCode::API_IDENTITY_ERROR);
        }
    ...
    public function loginDoc(string $appId, string $appSecret): int
    {
        ...
        if (! $model) {
            return MsProCode::API_PARAMS_ERROR;
        }

        if ($model->status != SystemApp::ENABLE) {
            return MsProCode::APP_BAN;
        }

        return MsProCode::API_VERIFY_PASS;
    }
    ...
    public function verifyEasyMode(string $appId, string $identity, array &$apiData): int
    {
        ...
        if (isset($apiData['app_id'])) {
            if ($apiData['app_id'] != $appId) {
                return MsProCode::API_UNBIND_APP;
            }
        ...
        if ($identity != md5($appId . $model->app_secret)) {
            throw new NormalStatusException(t('msproadmin.api_auth_fail'), MsProCode::API_SIGN_ERROR);
        }
