...
class SystemUploadFileService extends AbstractService
{
     ...
     * @var MsProUpload
     */
    protected MsProUpload $msproUpload;


    public function __construct(SystemUploadFileMapper $mapper, MsProUpload $msproUpload)
    {
        $this->mapper = $mapper;
        $this->msproUpload = $msproUpload;
    }
    ...
    public function upload(UploadedFile $uploadedFile, array $config = []): array
    {
        ...
        $data = $this->msproUpload->upload($uploadedFile, $config);
        if ($this->save($data)) {
            return $data;
        } else {
            return [];
    ...
    public function chunkUpload(array $data): array
    {
        ...
        $result = $this->msproUpload->handleChunkUpload($data);
    ...
    protected function handleArraySearch(Collection $collect, array $params): Collection
    {
        if ($params['name'] ?? false) {
            $collect = $collect->filter(function ($row) use ($params) {
                return \MsPro\Helper\Str::contains($row['name'], $params['name']);
            });
        }

        if ($params['label'] ?? false) {
            $collect = $collect->filter(function ($row) use ($params) {
                return \MsPro\Helper\Str::contains($row['label'], $params['label']);
            });
        }
        return $collect;
    }
    ...
    {
        $data = $this->msproUpload->handleSaveNetworkImage($data);
    ...
    public function responseFile(string $hash): \Psr\Http\Message\ResponseInterface
    {
        ...
        return container()->get(MsProResponse::class)->responseImage(
            $this->msproUpload->getFileSystem()->read(
                $this->msproUpload->getStorageMode() === '1'
